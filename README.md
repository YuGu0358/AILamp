...
The seven upstream LeLamp `.3mf` files are kept unchanged. The Jetson Nano hardware profile adds reversible, loose-fit adapter parts under `3D/AILamp_Adapters/` for the Jetson tray, electronics side deck, camera mount, NeoMatrix holder, ReSpeaker mount, and cable clips. Each adapter has a `.3mf` print file and a matching `.stl` export for slicer compatibility or visual checks.
